...
#' @param weight weight of the kNN values in the objective function, default = 0.5
...
#'                k = 15,
#'                distance = "gower")
biokNN.impute <- function(data, className, nIter = 10, weight = 0.5, k = 10, distance = "gower"){
  df_miss <- data
        ...
        for(i in index_miss[, j]){
          if(i != 0){
            clust_val <- clusters[i]
              ...
              M[i, j] <- weight*m_knn + (1-weight)*m_cluster
          }
        }
      }
      iter <- iter + 1
...
biokNN.impute.mi <- function(data, className, m =5, nIter = 10, weight = 0.5, k = 10, distance = "gower"){
  df_miss <- data
  if(check.data(df_miss, className)){
...
#' @param weight_space vector with the calibration values to test for the weight parameter
...
#'           k_space = c(10, 15),
#'           print = TRUE)
calibrate <- function(data, className, prop_valid = 0.1, nIter = 10, distance = "gower", weight_space = NULL, k_space = NULL, print = FALSE){
  df_miss <- data
  if(check.data(df_miss, className)){
    df_miss <- dplyr::select(df_miss, className, dplyr::everything())}

  if(is.null(weight_space)){
